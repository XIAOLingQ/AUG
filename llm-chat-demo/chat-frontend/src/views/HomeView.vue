  ...
  methods: {
    async handleSearch() {
      ...
      try {
        ...
        this.eventSource = new EventSource('http://27.25.158.240:21561/llm/request?query=' + keyword);
        // 设置消息事件监听器
      ...
      this.$nextTick(() => {
        ...
        codeBlocks.forEach(block => {
          ...
          button.onclick = async () => {
            ...
            // 发送 POST 请求到指定的 URL
            try {
              const response = await fetch('http://27.25.158.240:21561/get_uml', {
                method: 'POST',
                headers: {
    ...
    async resetMessages() {
      try {
        const response = await fetch('http://27.25.158.240:21561/reset_messages', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          }
