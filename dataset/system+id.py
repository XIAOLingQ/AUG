...
for index, item in enumerate(data):
    ...
    #item['instruction'] = "评分标准：• Completeness (完整性): 图表涵盖了所有需求的内容（外部完整性 [31]），并且具有足够的细节以便与潜在的利益相关者进行沟通。 • Correctness (正确性): 图表指定的行为与需求一致且连贯。 • Adherence to the standard (遵循标准): 图表在语法上是正确的（即，它可以被 PlanText6 解释），在语义上也是合理的（即，使用了适当的结构）。 • Degree of understandability (可理解度): 鉴于需求的复杂性，图表足够清晰，并且没有包含冗余内容。 • Terminological alignment (术语对齐): 生成的图表中使用的术语与需求中的术语一致。 根据用户提供的需求用户故事文档，生成相应的uml图，并根据提供的评估标准对生成的图进行评分。"
